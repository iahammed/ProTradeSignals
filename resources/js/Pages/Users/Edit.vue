    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!user.deleted_at && $page.props.auth.user.permissions.delete_user" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete User</button>
    ...
    <div class="bg-white rounded-md shadow overflow-x-auto mt-10">
      <table class="w-full whitespace-nowrap">
        <tr class="text-left font-bold">
          ...
          <th class="pb-4 pt-6 px-6">Finish Date</th>
          <th class="pb-4 pt-6 px-6">processed</th>
        </tr>
        <tr v-for="subscription in subscriptions" :key="subscription.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t"><p class="flex items-center px-6 py-4">{{ subscription.starts }}</p></td>
          <td class="border-t"><p class="flex items-center px-6 py-4">{{ subscription.ends }}</p></td>
          <td class="border-t"><p class="flex items-center px-6 py-4">{{ subscription.processed }}</p></td>
        </tr>
      </table>
    </div>
  </div>
